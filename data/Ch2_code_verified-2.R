# Fletcher and Fortin 2018 - Chapter 2: Scale ----
...
library(rgeos)       #for buffer analysis; version 0.5-8 used
library(fields)      #for kernel analysis; version 13.3 used
library(Matrix)      #for kernel analysis, version 1.3-4 used
...
nlcd <- raster(file.path(data_dir, "nlcd2011SE.nc"))
...
sites = readOGR(file.path(data_dir, "reptiledata"))
...
# herp data
flsk = read.csv(file.path(data_dir, "reptiles_flsk.csv"))
...
#rename
names(forest200.df) <- c("x", "y", "layer")
...
# NOTE: this is very slow - only run it the first time, then save to a file

if(FALSE)
{
  
  lr.kernel <- optim(fn=nll.kernel, hessian=T, method='L-BFGS-B',
                     D=D, par=c(0,-6,8.5),                              #use starting values based on lr.buffer, adjusted for proportion rather than % cover
                     cov=forest200.df$layer[cov.subset], y=flsk$pres)
  # Save 
  save(lr.kernel, file = here("data", "ch_02_lr_kernel.RData"))
}

load(here("data", "ch_02_lr_kernel.RData"))
