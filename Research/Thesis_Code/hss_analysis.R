...
data_dir
df = read.csv(file.path("data", "percentCorrectandConfidence.csv"), sep=",")
head(df)

#To Plot the correctness graph after the sheet has been made in excel 
ggplot(df, aes(x = Name, y = Percent.Correct, fill = TF)) +  geom_bar(stat="identity", position="dodge") + ggtitle("Proportion of correct decision by situation") + theme(
  axis.text.x=element_blank(),axis.ticks.x=element_blank()) + ylim(0,1) + xlab("Image Viewed")

#Plotting the confidence per situation
ggplot(df, aes(x = Name, y = Average.confidence., fill = TF)) +  geom_bar(stat="identity", position="dodge") + ggtitle("Proportion of confidence by decision") + theme(
  axis.text.x=element_blank(),axis.ticks.x=element_blank()) + ylim(0,1) + xlab("Image Viewed")

df2 = df[!(is.na(df$condition) | !(df$variable == "score")),]
df2conf = df[!(is.na(df$condition) | !(df$variable == "confidence")),]
...
#percent for confidence
df_percent = df2conf[c("subID", "event", "value", "variable", "condition", "image")]

#percent for trade
df_percent = df_trade[c("subID", "event", "value", "variable", "condition", "image")]
...
#New for confidence. Not in real use
final3 = df_conf[(df_detm$orderSeen == 72| df_detm$orderSeen == 140| df_detm$orderSeen == 67| df_detm$orderSeen == 135),]
...
#Real calculation of true or false on what the confidence is
...
write.csv(df_percent,"df_hss_all_trade.csv", row.names = FALSE)
#need to do stuff in excel after this
