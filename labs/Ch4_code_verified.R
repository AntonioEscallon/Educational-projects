...
# ---- 4.3.4 Univariate point patterns ----
{
  ...
  #isotropic edge correction (0 expectation)
...
# ---- 4.3.6 Inhomogenous point processes ----
{
  ...
  veg.height <- read.csv(
    file.path(data_dir, 'cactus_matrix.csv'), header=T)
  ...
  veg.height.mat[] <- as.numeric(veg.height$Height)
  ...
  plot(ppp.cactuss, add=T)
  
  #Inhomogeneous L function that accounts for this trend
  pp.xy.pred <- predict.ppm(pp.xy2, type="trend")
  Lxycsr <- envelope(ppp.cactuss, Linhom, nsim=99, rank=1, correction="translate", simulate=expression(rpoispp(pp.xy.pred)), global=FALSE)
  ...
  Kmatern <- kppm(ppp.cactus, ~1, "MatClust")
